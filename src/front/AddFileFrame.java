	...
	public AddFileFrame() {
		...
		confirm.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				DefaultMutableTreeNode node=(DefaultMutableTreeNode)MainFile.tree.getLastSelectedPathComponent();
				Package_ p=(Package_)node.getUserObject();
				String type=textField_type.getText().trim();
				...
				if(!type.equals("")) name+="."+type;
				if(p.check(name)){
					File_ f=new File_(name, size);
					if(!f.exists()) {
					...
					node.add(newnode);
					SwingUtilities.updateComponentTreeUI(MainFile.tree);
