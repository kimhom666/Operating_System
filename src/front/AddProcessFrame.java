...
public class AddProcessFrame extends JFrame {
	...
	public AddProcessFrame(String pname) {
		this.setTitle(pname);
		...
		vector=new Vector<>();
		name=new Vector<>(Arrays.asList("时间片","设备名","设备数量"));
		tp= new JScrollPane();
		add = new JButton("添加行");
		...
		confirm.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(MemoryController.virtualMemory)
				{
					...
					if(ProcessController.add(pri,pname,request,pagelist)==false) 
					...
					if(ProcessController.add(pri, pname,cost, request, msize)==false)
						JOptionPane.showMessageDialog(null, "无法给该进程分配内存或资源",null, JOptionPane.ERROR_MESSAGE);
				}
				setVisible(false);
				MainProcess.mainProcess.update();
			}
		});
